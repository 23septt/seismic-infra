...
from . import SeismicReading

log = logging.getLogger(__name__)

# LSM6DSOX register map
_CTRL1_XL     = 0x10   # accel control
_CTRL2_G      = 0x11   # gyro control (not used here)
_STATUS_REG   = 0x1E
_OUTZ_L_A     = 0x2C
_OUTZ_H_A     = 0x2D
_WHO_AM_I     = 0x0F
_WHO_AM_I_VAL = 0x6C   # expected value for LSM6DSOX

# ODR=52Hz, ±2g
_CTRL1_ODR52_2G = 0x30
...
class SeismicSensor:
    ...
    def _init_device(self) -> bool:
        try:
            who = self._board.i2c_read_bytes(config.LSM6DSOX_SEISMIC_ADDR, _WHO_AM_I, 1)[0]
            if who != _WHO_AM_I_VAL:
                log.warning("LSM6DSOX WHO_AM_I=0x%02X (expected 0x%02X)", who, _WHO_AM_I_VAL)
            self._board.i2c_write_bytes(config.LSM6DSOX_SEISMIC_ADDR, _CTRL1_XL,
                                        bytes([_CTRL1_ODR52_2G]))
            return True
        except OSError as e:
            log.error("LSM6DSOX init failed: %s", e)
            return False

    def _read_z_raw(self) -> Optional[float]:
        try:
            data = self._board.i2c_read_bytes(config.LSM6DSOX_SEISMIC_ADDR, _OUTZ_L_A, 2)
            raw = struct.unpack('<h', data)[0]
            return raw * config.LSM6DSOX_SENSITIVITY
        except OSError:
            return None
    ...
    def _loop(self) -> None:
        if not self._init_device():
            log.error("Seismic sensor unavailable")
            self._stop.wait(5)
            return
